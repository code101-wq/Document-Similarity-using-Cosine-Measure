...
def Form_vectors():
    ...
    for stringss in d:  # loop through the list of dictionaries
        ...
        i = i + 1
    print(i)
    print(dict_list)  # dict_list is a list of dictionaries

    #  Check whether word in a particular document exists in the set of words
    j = 0
    doc_collection = []
    for l in dict_list:  # l here is a dictionary in the dict_list
        doc = []
        keys = l.keys()  # collect the keys of the dictionary l and save it in keys
...
def Create_Cluster(a, b):
    #  Clustering is based on documents with similarity score above 0.2
    i = 0
    j = 0
    for i in range(len(a)+1):
        ...
        for j in range(len(a[i])+1):
            if j > len(a[i])-1 or i > len(a) - 1:
                break
            if a[i][j] > 0.2 and b[i] != b[j]:
                sim_cluster.append(b[i])
                sim_cluster.append(b[j])
            if a[i][j] <= 0.2:
                dissim_cluster.append(b[i])
                dissim_cluster.append(b[j])
